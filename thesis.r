...
read_csv("leverage.csv") %>% inner_join(read_csv("major_customer.csv")) %>% inner_join(read_csv("bm.csv")) %>% 
inner_join(read_csv("e_index.csv")) %>% inner_join(read_csv("eqinc.csv")) %>% inner_join(read_csv("female_ceo.csv")) %>% 
inner_join(read_csv("growth.csv")) %>% inner_join(read_csv("intangible.csv")) %>% inner_join(read_csv("pay_turn.csv")) %>% 
inner_join(read_csv("roa.csv")) %>% inner_join(read_csv("size.csv")) %>% inner_join(read_csv("tangibility.csv")) %>% 
separate(col = gvkey_fyear, into = c("gvkey", "fyear"), sep = "_") %>% 
write_csv("sample_3.csv")
